...
export default function InternshipsSection({
  ...
  const paginatedItems = internships.slice(
    (safeCurrentPage - 1) * ITEMS_PER_PAGE,
    safeCurrentPage * ITEMS_PER_PAGE
  );

  return (
    <div id="internships-section" className="space-y-3.5 pt-1">
      {/* Section Header */}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-[#55786B] dark:text-[#8FAFA4] mt-0.5">
              Gain hands-on experience, work on real projects, and grow your career.
            </p>
          </div>
        </div>
      ...
      {paginatedItems.length === 0 ? (
        <div className="w-full py-12 px-4 rounded-2xl bg-[#FFFFFF] dark:bg-[#06241F] border border-[#D8E8E2] dark:border-[#16463D] flex flex-col items-center justify-center text-center shadow-xs">
          ...
          <p className="text-xs text-[#55786B] dark:text-[#8FAFA4] mt-1 max-w-xs">
            No internship opportunities match your active filters or search keyword.
          </p>
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 items-stretch">
          {paginatedItems.map((item) => (
            <OpportunityCard
              ...
              onApplyOrRegister={onApply}
              onToggleBookmark={onToggleBookmark}
              isSaved={savedIds.has(item.id)}
              isAppliedOrRegistered={appliedIds.has(item.id)}
            />
          ))}
        </div>
      )}
      ...
      {internships.length > ITEMS_PER_PAGE && (
